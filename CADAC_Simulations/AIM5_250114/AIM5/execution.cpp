...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_aim,int num_aircraft,ofstream &ftraj,char *run_title,bool traj_merge);
...
void merge_plot_files(string *plot_file_list,int num_aim,char *run_title);

//writing 'combus' data on screen
void comscrn_data(Packet *combus,int num_vehicles);

//writing banner on 'traj.asc' file 
//void traj_banner(ofstream &ftraj,Packet *combus,char *run_title,int num_vehicles);
void traj_banner(ofstream &ftraj,Packet *combus,char *run_title,int num_vehicles,
...
int main() 
{
	...
	int num_aim=0; //number of aim objects
	...
	for(int i=0;i<num_vehicles;i++)
	{
		...
		if(strstr(options,"y_doc"))
		{
			if(!strcmp(vehicle_name,"AIM5")&&document_aim)
			{
				...
				vehicle_list[i]->document(fdoc,run_title,doc_aim5);
				document_aim5=true;
			}
			if(!strcmp(vehicle_name,"AIRCRAFT3")&&document_aircraft)
			{
				...
				vehicle_list[i]->document(fdoc,run_title,doc_aircraft3);
	...
	if(strstr(options,"y_traj")&&one_traj_banner)
	{
		...
		traj_banner(ftraj,combus,run_title,num_vehicles,num_aim,num_aircraft);
	...
	execute(vehicle_list,module_list,sim_time,
				...
				num_aim,num_aircraft,ftraj,run_title,traj_merge);
	...
	{
		merge_plot_files(plot_file_list,num_aim,run_title);
	}
			...
			if(health==1)
			{
				...
				for(int j=0;j<num_modules;j++)
				{
					if(module_list[j].name=="environment")
						...
						vehicle_list[i]->intercept(combus,vehicle_slot,int_step,run_title);
				} //end of module loop
