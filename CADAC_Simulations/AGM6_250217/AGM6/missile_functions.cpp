...
//011129 Adapted to MISSILE6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
void Missile::tabout_banner(ofstream &ftabout,const char * title,int &nmonte,int &nmc)
{
	const char * buff1;
	char buff2[15];
	int label_length=14;
...
//			aero-tables ('Missile' data members)
//			prop-tables ('Missile' data members)
...
//020723 Included and initialized Markov 'saved' value
///////////////////////////////////////////////////////////////////////////////
void Missile::vehicle_data(fstream &input,int nmonte)
{
	...
	do
	{
		...
		else
		{
			...
			if(!strcmp(read,"UNI"))
			{
				...
				for(kk=0;kk<NFLAT6;kk++)
				{
					buff=flat6[kk].get_name();
				...
				for(kk=0;kk<NMISSILE;kk++)
				{
					buff=missile[kk].get_name();
					if(!strcmp(buff,name1)) 
					{
						missile[kk].gets(value);
						input.getline(line_clear,CHARL,'\n');
					}				
				}
			}
			...
			if(!strcmp(read,"MARKOV"))
			{
				...
				for(ii=0;ii<NFLAT6;ii++)
				{
					buff=flat6[ii].get_name();
					if(!strcmp(buff,name1)) 
					{
						markov_list[nmarkov].set_markov_flat6_index(ii);
						flat6[ii].gets(value);
						input.getline(line_clear,CHARL,'\n');
					}				
				}
...
///////////////////////////////////////////////////////////////////////////////
void Missile::plot_banner(ofstream &fplot,const char * title)
{
	const char * buff1;
	char buff2[15];
	int label_length=13;
...
//011026 Making event_epoch a 'Missile' member for 'stat.asc' file usage, PZi
//011129 Adapted to MISSILE6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
void Missile::event(const char * options)
{
...
///////////////////////////////////////////////////////////////////////////////
void Missile::document(ostream &fdoc,const char * title,Document *doc_missile6)
{
	fdoc<<"*********************************************************************************************************************\n";
	fdoc<<"********************************************** MISSILE6 **************************************************************\n";
	fdoc<<"*********************************************************************************************************************\n";
...
//011129 Adapted to MISSILE6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
Packet Missile::loading_packet_init(int num_missile,int num_aircraft,int num_target)
{
	string id;
	char object[4];
	...
	for(int j=0;j<missile_com_count;j++)
	{
...
//011129 Adapted to MISSILE6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
Packet Missile::loading_packet(int num_missile,int num_aircraft,int num_target)
{
	int index;
	int i(0);
...
// 
//
//010916 Created by Peter H Zipfel
//011129 Adapted to MISSILE6 simulation, PZi
//020723 Included 'saved' value, PZi
///////////////////////////////////////////////////////////////////////////////
void Missile::markov_noise(double time,double int_step,int nmonte)
{
	int index_flat6;
	int index_missile;
	double sigma;
	...
	{
		index_flat6=markov_list[i].get_markov_flat6_index();
		index_missile=markov_list[i].get_markov_vehicle_index();
		if(index_flat6!=ILARGE)
		{
			...
			if(status)
				flat6[index_flat6].gets(markov(sigma,bcor,time,int_step,saved));
				markov_list[i].set_markov_saved(flat6[index_flat6].real()); 
				if(!nmonte) flat6[index_flat6].gets(null);
		}
		if(index_missile!=ILARGE)
		{
			sigma=markov_list[i].get_markov_sigma();
			bcor=markov_list[i].get_markov_bcor();
			saved=markov_list[i].get_markov_saved();
			status=markov_list[i].get_markov_status();
			if(status)
				missile[index_missile].gets(markov(sigma,bcor,time,int_step,saved)); 
				markov_list[i].set_markov_saved(missile[index_missile].real());
				if(!nmonte) missile[index_missile].gets(null);
		}
	}
}
///////////////////////////////////////////////////////////////////////////////
...
//011129 Adapted to MISSILE6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
void Missile::stat_data(ofstream &fstat,int nmc,int vehicle_slot)
{
	int index;
	const char * integer;
	const char * vector;
	...
	//writing to 'stati.asc' the variables from the 'Flat6' class
	for(i=0;i<flat6_plot_count;i++)
	{
		index=flat6_plot_ind[i];
		//checking for integers
		...
		if(!strcmp(integer,"int"))
		{
			...
			if(k>4){k=0;fstat<<'\n';}
			fstat.width(16);
			fstat<<(double) flat6[index].integer();			
			k++;
		}
		//checking vor vectors
		else if(isupper(vector[0]))
		...
		else
		{
			if(k>4){k=0;fstat<<'\n';}
			fstat.width(16);
			fstat<<flat6[index].real(); 
			k++;
		}
	}
	//writing to 'stati.asc' the variables from the 'Missile' class
	for(i=0;i<missile_plot_count;i++)
	{
		index=missile_plot_ind[i];
		//checking for integers
		...
		if(!strcmp(integer,"int"))
		{
			...
			if(k>4){k=0;fstat<<'\n';}
			fstat.width(16);
			fstat<<(double) missile[index].integer();			
			k++;
...
///////////////////////////////////////////////////////////////////////////////
void Missile::read_tables(const char * file_name,Datadeck &datatable)
{
