...
//100405 Modified for AGM6, PZi
//140824 Upgraded to Visual C++ V12 (2013), PZi
//191008 Output in 'csv' format added, PZi
///////////////////////////////////////////////////////////////////////////////
...
void number_objects(fstream &input,int &num_vehicles,int &num_missile,int &num_target,int &num_aircraft);

//creating a type of vehicle object
Cadac *set_obj_type(fstream &input,Module *module_list,int num_modules,
				   int num_target,int num_aircraft);
...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_missile,int num_target,int num_aircraft,ofstream &ftraj,const char * title,bool traj_merge,
...
void traj_banner(ofstream &ftraj,Packet *combus,const char * title,int num_vehicles,
				 int nmissile,int ntarget,int naircraft);
...
void document_input(Document *doc_missile6,Document *doc_target3,Document *doc_aircraft3);
...
int main() 
{
	...
	do
	{
		...
		number_objects(input,num_vehicles,num_missile,num_target,num_aircraft);
		...
		{
			//Loading pointers of the vehicle-object classes ('Missile','Target'.'Aircraft') into 
			...
			//Furthermore, it passes 'module_list', 'num_modules','num_target'and'num_aircraft'
			// to the 'Missile', 'Target' and 'Aircraft' constructors
			vehicle_type=set_obj_type(input,module_list,num_modules,num_target,num_aircraft);
			...
			{		
				...
				//composing documentation of 'flat6','missile','flat3','target' and 'aircraft' module-variables
				//but do it only once for MISSILE6, TARGET3 and AIRCRAFT3 objects
				if(strstr(options,"y_doc"))
				{
					...
					if(!strcmp(vehicle_name,"TARGET3")&&document_target)
					{
						...
						catch(bad_alloc xa){cerr<<"*** Allocation failure of 'doc_target3' *** \n";return 1;}
			...
			combus[i]=vehicle_list[i]->loading_packet_init(num_missile,num_aircraft,num_target);
		...
		if(strstr(options,"y_traj")&&one_traj_banner)
		{
			...
			traj_banner(ftraj,combus,title,num_vehicles,num_missile,num_target,num_aircraft);
		...
		if(!nmc&&strstr(options,"y_doc")){
			...
			document_input(doc_missile6,doc_target3,doc_aircraft3);
		...
		execute(vehicle_list,module_list,sim_time,
				 ...
				 plot_ostream_list,combus,status,num_missile,num_target,num_aircraft,ftraj,title,
...
//								and their respective type (missile, target, or aircraft)
				...
				if(health==1)
				{
					...
					for(int j=0;j<num_modules;j++)
					{
						if(module_list[j].name=="environment")
							...
							vehicle_list[i]->sensor(combus,num_vehicles,sim_time,int_step);
						else if(module_list[j].name=="intercept")
							vehicle_list[i]->intercept(combus,vehicle_slot,num_vehicles,int_step,title);
					} //end of module loop
					...
					combus[i]=vehicle_list[i]->loading_packet(num_missile,num_aircraft,num_target);
