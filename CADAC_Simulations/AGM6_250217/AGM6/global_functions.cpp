...
//191008 Included csv option, PZi
///////////////////////////////////////////////////////////////////////////////
...
void number_objects(fstream &input,int &num_vehicles,int &num_missile,int &num_target,int &num_aircraft)
{
	...
	//searching for # of missile, target and aircraft objects
	...
	}while((num_missile+num_target+num_aircraft)<num_vehicles);
...
//Arguments of object: module_list, num_modules, num_target, num_aircraft to be passed 
// to the constructorof 'Missile', 'Target' and 'Aircraft'
...
Cadac *set_obj_type(fstream &input,Module *module_list,int num_modules,int num_aircraft,int num_target)				   
{
	char line_clear[CHARL];
	char temp[CHARN];
	Cadac *obj=NULL;
				 ...
				 int nmissile,int ntarget,int naircraft)
{
	...
	//number of data elements in 'com_missile', 'com_target' and 'com_aircraft'
	...
	//determining the total number of vector variables in com_missile6, com_target3 and com_aircraft3
	...
	int nvariables=ncom_missile6*nmissile+ncom_target3*ntarget+ncom_aircraft3*naircraft
		+2*(p*nmissile+q*ntarget+r*naircraft)-(nmissile-1)-ntarget-naircraft;//z070426 last two terms added to remove the 'time' count
	...
	for(i=0;i<num_vehicles;i++)
	{
		//determining wether 'Missile', 'Target' or 'aircraft' object
		id=combus[i].get_id();
//		loc=id.find("m"); 
		if(!id.find("m"))
		//'Missile' object
...
void traj_data(ofstream &ftraj,Packet *combus,int num_vehicles,bool merge)
{
	...
	{
		//determining wether 'Missile', 'Aircraft', or 'Target' object
		id=combus[i].get_id();
		missile_object=int(id.find("m"));
		target_object=int(id.find("t"));
		aircraft_object=int(id.find("a"));
		if(!missile_object)
		//'Missile' object
		{
...

{
	...
	//determining the number of missile, target, and aircraft packets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=int(id.find("m"));
	...
	//'Target' object labels (excluding 'time') 
...
///////////////////////////////////////////////////////////////////////////////
void Variable::init(const char * na,double v1,double v2,double v3,double v4,double v5,
					const char * de,const char * mo,const char * ro,const char * ou)
{
	double *pbody;
...
//not used:	if(!strcmp(name,"empty")==0) error[0]='*';
	strcpy(name,na);
	strcpy(def,de);
	strcpy(mod,mo);
	strcpy(role,ro);
...
void Variable::init(const char * na,
...
//	}while(strcmp(buffl,"STOP"));
	...
	input.setf(ios::left);
	do{
		fcopy>>buffo;;//buffo gets MISSILE6, TARGET3, aircraft3, ENDTIME STOP and //comments
...
//////////////////////////////////////////////////////////////////////////////
void parse_plot_traj_csv(string *files, int num_missile, bool merge, string type)
{
