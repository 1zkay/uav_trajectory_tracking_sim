...
//		= 2 starting star track upates
//		= 3 sending tilt corrections to INS ('ins' module resets mstar=2) 
//
//040212 Created by Peter H Zipfel
//230706 Diagnostics to display stars, PZi
///////////////////////////////////////////////////////////////////////////////
...
void Hyper::def_startrack()
{
	...
	hyper[816].init("el1_bias",0,"Star elevation error 1 bias - rad GAUSS","startrack","data","");
	hyper[817].init("el2_bias",0,"Star elevation error 2 bias - rad GAUSS","startrack","data","");
	hyper[818].init("el3_bias",0,"Star elevation error 3 bias - rad GAUSS","startrack","data","");
	hyper[819].init("el1_noise",0,"Star elevation error 1 noise - rad MARKOV","startrack","data","");
	hyper[820].init("el2_noise",0,"Star elevation error 2 noise - rad MARKOV","startrack","data","");
	hyper[821].init("el3_noise",0,"Star elevation error 3 noise - rad MARKOV","startrack","data","");
	hyper[830].init("URIC",0,0,0,"Tilt corrections for INS","startrack","out","plot");
	//diagnostics
	hyper[831].init("starlon1",0,"Longitude of 1st star - deg","startrack","diag","");
	hyper[832].init("starlat1",0,"Latitude of 1st star - deg","startrack","diag","");
	hyper[833].init("staralt1",0,"Altitude of 1st star - m","startrack","diag","");
	hyper[834].init("starlon2",0,"Longitude of 2st star - deg","startrack","diag","");
	hyper[835].init("starlat2",0,"Latitude of 2st star - deg","startrack","diag","");
	hyper[836].init("staralt2",0,"Altitude of 2st star - m","startrack","diag","");
	hyper[837].init("starlon3",0,"Longitude of 3st star - deg","startrack","diag","");
	hyper[838].init("starlat3",0,"Latitude of 3st star - deg","startrack","diag","");
	hyper[839].init("staralt3",0,"Altitude of 3st star - m","startrack","diag","");
}
///////////////////////////////////////////////////////////////////////////////  
//Star tracker module
//Member function of class 'Hyper'
...
void Hyper::startrack()
{
	...
	double starlon1=hyper[831].real();
	double starlat1=hyper[832].real();
	double staralt1=hyper[833].real();
	double starlon2=hyper[834].real();
	double starlat2=hyper[835].real();
	double staralt2=hyper[836].real();
	double starlon3=hyper[837].real();
	double starlat3=hyper[838].real();
	double staralt3=hyper[839].real();
	...
	if(mstar==1)
	{
		...
		star_init(star_data,star_names);
	...
	if(mstar==3)
	{
		...
		for(int i=0;i<3;i++){
			...
			*(slot+i)=*(usii_triad+4*i+3);
			...
			//diagnostics to display star triad on globe, PZI 230706
			//void cad_geo84_in(double &lon,double &lat,double &alt, const Matrix SBII,const double &time);
			double staralt=700000;
			if(i==0) {
			cad_geo84_in(starlon1,starlat1,staralt1,USII,time); staralt1=staralt;
			starlon1*=DEG;starlat1*=DEG;
			}
			if(i==1) {
			cad_geo84_in(starlon2,starlat2,staralt2,USII,time); staralt2=staralt;
			starlon2*=DEG;starlat2*=DEG;
			}		
			if(i==2) {
			cad_geo84_in(starlon3,starlat3,staralt3,USII,time); staralt3=staralt;
			starlon3*=DEG;starlat3*=DEG;
			}
		}	
	...
	hyper[831].gets(starlon1);
	hyper[832].gets(starlat1);
	hyper[833].gets(staralt1);
	hyper[834].gets(starlon2);
	hyper[835].gets(starlat2);
	hyper[836].gets(staralt2);
	hyper[837].gets(starlon3);
	hyper[838].gets(starlat3);
	hyper[839].gets(staralt3);
}
...
void Hyper::star_init(double *star_data,string *star_names)
{
	int i(0);
