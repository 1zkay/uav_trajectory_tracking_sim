...
//Module-variable locations are assigned to hyper[300-399]
//
...
void Hyper::def_ins()
{
	...
	hyper[346].init("EVBI",0,0,0,"INS vel error - m/s","ins","state","plot");
	hyper[347].init("ESBID",0,0,0,"INS pos error derivative - m","ins","state","");
	hyper[348].init("ESBI",0,0,0,"INS pos error - m","ins","state","plot");
	hyper[349].init("ins_pos_err",0,"INS absolute postion error - m","ins","diag","scrn,plot");
	hyper[350].init("ins_vel_err",0,"INS absolute velocity error - m/s","ins","diag","scrn,plot");
	hyper[351].init("ins_tilt_err",0,"INS absolute tilt error - rad","ins","diag","scrn,plot");
	hyper[352].init("frax_transfer",0,"Initial P-matrix change P=P(1+frax_transfer)","ins","data","");
	hyper[353].init("eunbg",0,"Gyro cluster misalignment test - rad","ins","data","");}	
...
void Hyper::init_ins()
{
	...
	double frax_transfer=hyper[352].real();
	...
	if(mins==0){
		//do nothing
			...
			//Initial covariance matrix  (GPS quality transfer)
			...
			//changing the transfer covariance matrix
			PP_INIT*=(1+frax_transfer);
...
void Hyper::ins(double int_step)
{
	...
	if(VBECB[1]==0 && VBECB[2]==0)
		phipc=0.;
	//note: if vbeb2 is < EPS (value) then phipc is forced to be 0 or PI
	...
	hyper[349].gets(ins_pos_err);
	hyper[350].gets(ins_vel_err);
	hyper[351].gets(ins_tilt_err);
}	
///////////////////////////////////////////////////////////////////////////////
//Gyro error model
//Member function of class 'Hyper'
...
Matrix Hyper::ins_gyro(Matrix &WBICB, double int_step)
{
	...
	//diagnostics: gyro cluster misalignment test - 230705 PZi
	EUNBG.assign_loc(0,0,eunbg);
	EUNBG.assign_loc(1,0,eunbg);
	EUNBG.assign_loc(2,0,eunbg);
	//end dignostics
...
// Ref: Zipfel, Section 10.2.4.3, Eq.10.94
// 
//Return output
