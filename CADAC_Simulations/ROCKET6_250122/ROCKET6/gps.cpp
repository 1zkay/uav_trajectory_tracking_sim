...
void Hyper::def_gps()
{
	...
	//Z150126  Plotting of quadriga
	hyper[727].init("lon1",0,"Longitude of 1st SV - deg","gps","diag","");
	hyper[728].init("lat1",0,"Latitude of 1st SV - deg","gps","diag","");
	hyper[729].init("alt1",0,"Altitude of 1st SV - m","gps","diag","");
	hyper[730].init("lon2",0,"Longitude of 2st SV - deg","gps","diag","");
	hyper[731].init("lat2",0,"Latitude of 2st SV - deg","gps","diag","");
	hyper[732].init("alt2",0,"Altitude of 2st SV - m","gps","diag","");
	hyper[733].init("lon3",0,"Longitude of 3st SV - deg","gps","diag","");
	hyper[734].init("lat3",0,"Latitude of 3st SV - deg","gps","diag","");
	hyper[735].init("alt3",0,"Altitude of 3st SV - m","gps","diag","");
	hyper[736].init("lon4",0,"Longitude of 4st SV - deg","gps","diag","");
	hyper[737].init("lat4",0,"Latitude of 4st SV - deg","gps","diag","");
	hyper[738].init("alt4",0,"Altitude of 4st SV - m","gps","diag","");
	hyper[739].init("dum_alt",0,"Dummy alt for plotting - m","gps","diag","");
	//GPS filter
	...
	hyper[778].init("c2_pos_meas",0,"SV C2 position measurement residuals - m","gps","diag","plot");
	hyper[779].init("c2_vel_meas",0,"SV C2 velocity measurement residuals - m/s","gps","diag","plot");
	hyper[780].init("state_pos",0,"State x absolute position value - m","gps","diag","plot");
	hyper[781].init("state_vel",0,"State x absolute velocity value - m","gps","diag","plot");
	hyper[782].init("c2_range_err",0,"Position meas error of SV C2 - m","gps","diag","plot");
	hyper[783].init("c2_delta_err",0,"Velocity meas error of SV C2 - m/s","gps","diag","plot");
}
///////////////////////////////////////////////////////////////////////////////  
//GPS module
//Member function of class 'Hyper'
...
void Hyper::gps(double int_step)
{
	...
	double c2_pos_meas=hyper[778].real();
	double c2_vel_meas=hyper[779].real();
	...
	//getting saved values of GPS measurement errors
	double c2_range_err=hyper[782].real();
	double c2_delta_err=hyper[783].real();	// 
	...
	if(mgps==2)
	{
		...
		//diagnostics: std. deviations of the diagonals of the covariance matrix
	...
	if(mgps==3)
	{
		...
		for(i=0;i<4;i++){
			...
			if(time>=740){dum_alt=(time-740)*1000;}			
			//Z150126 - end
			*/
			...
			//diagnostic output for SV C2 - 230628 ZI
			if(i==0)
			{
			...
			//diagnostic output for SV C2 - 230628 ZI
			if(i==0) {c2_delta_err=dvsb_meas-dvsb;};
		...
		c2_pos_meas=ZZ.get_loc(0,0);
		c2_vel_meas=ZZ.get_loc(4,0);
...
void Hyper::gps_sv_init(double *sv_init_data,double &rsi,double &wsi,double &incl)
{
	...
	//inclination constant for all SVs = 55deg = 0.95986rad
	incl=0.95986;
}
