...
protected:
	...
	//Array of module-variables as defined in class 'Rocket'
	//first ten locations are reserved for 'com' variables
	Variable *rocket=nullptr;

	//Array of module-variables as defined in class 'Aircraft'
	//first ten locations are reserved for 'com' variables
	Variable *aircraft=nullptr;

	//Array of module-variables as defined in class 'Radar'
	//first ten locations are reserved for 'com' variables
	Variable *radar=nullptr;
...
public:
	...
	virtual Packet loading_packet_init(int num_missile,int num_aircraft,int num_rocket,int num_radar)=0;
	virtual Packet loading_packet(int num_missile,int num_aircraft,int num_rocket,int num_radar)=0;
	...
	virtual void kinematics(double sim_time,double event_time,double &int_step,double &out_fact,Packet *combus,int num_vehicles,int vehicle_slot)=0;
	...
	virtual void rcs(double int_step)=0;
	virtual void def_control()=0;
	virtual void control(double int_step)=0;
	virtual void def_guidance()=0;
	virtual void guidance(Packet *combus,int num_vehicles,int vehicle_slot,double int_step)=0;
	virtual void def_ins()=0;
	virtual void init_ins()=0;
	virtual void ins(double int_step)=0;
	virtual void def_sensor()=0;
	virtual void sensor(Packet *combus,int num_vehicles,int vehicle_slot,double sim_time,double int_step)=0;
...
public:
	...
	virtual void kinematics(double sim_time,double event_time,double &int_step,double &out_fact,Packet *combus,int num_vehicles,int vehicle_slot);
...
protected:
	...
	//array of ground distances of 'Missile' object from all 'Rocket' objects
...
public:
	Missile(){};
	Missile(Module *module_list,int num_modules,int num_rocket);
	virtual~Missile();
	...
	virtual Packet loading_packet_init(int num_missile,int num_aircraft,int num_rocket,int num_radar);
	virtual Packet loading_packet(int num_missile,int num_aircraft,int num_rocket,int num_radar);
	...
	virtual void rcs(double int_step);
	virtual void def_control();
	virtual void control(double int_step);
	virtual void def_guidance();
	virtual void guidance(Packet *combus,int num_vehicles,int vehicle_slot,double int_step);
	virtual void def_ins();
	virtual void init_ins();
	virtual void ins(double int_step);
	virtual void def_sensor();
	virtual void sensor(Packet *combus,int num_vehicles,int vehicle_slot,double sim_time,double int_step);
	...
	Matrix ins_accl();
	void ins_alt();
	Matrix guidance_mid_pronav(Matrix STBLC);
	...
	void sensor_ir_dyn(int &skr_mode,int &mguide,double &thtpb,double &psipb,double &sigdy,
	...
	void sensor_kin(double &thtpb,double &psipb,double &sigdy,double &sigdz,double &lamdrb,double &lamdqb,double &ddab,
...
//Models rocket movements
//Contains Module 'forces'
//
//010205 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////

class Rocket:public Flat3
{
protected:
	//name of ROCKET5 vehicle object
	char rocket3_name[CHARL]="";

	//Communications output array of module-variables of vehicle object 'Rocket'
	Variable *com_rocket3=nullptr;int ncom_rocket5=0;

	//Packet of data for each rocket vehicle
	Packet packet;

	//Indicator array pointing to the module-variable which are to 
	//be written to 'combus' 'packets'
	int *rocket_com_ind=nullptr; int rocket_com_count=0;
...
public:
	Rocket(){};
	Rocket(Module *module_list,int num_modules);
	virtual~Rocket();
...
//
//Second level of derived class of the 'Cadac' hierarchy, branching from 'Flat3'
//Models aircraft flight path and rocket track files
//Contains Module 'forces'
//
...
protected:
	...
	Variable *com_aircraft3=nullptr;int ncom_aircraft3=0;
	...
	//Indicator array pointing to the module-variable which are to 
	//be written to 'combus' 'packets'
	int *aircraft_com_ind=nullptr; int aircraft_com_count=0;
...
//
//First derived class in the 'Cadac' hierarchy
//Contains modules: 'kinematics' and 'newton'
//
//170916 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
class Flat0:public Cadac
{
protected:			
	//Indicator array pointing to the module-variable which are to 
	//be written to 'combus' 'packets'
	int *flat0_com_ind=nullptr; int flat0_com_count=0;
public:
	Flat0();
	virtual~Flat0(){};
...
//Derived class:Radar
//
//Second level of derived class of the 'Cadac' hierarchy, branching from 'Flat0'
//Models tracking radar
//
//170916 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
class Radar:public Flat0
{
protected:
	//name of RADAR0 vehicle object
	char radar0_name[CHARL]={};

	//Communications output array of module-variables of vehicle object 'Radar'
	Variable *com_radar0=nullptr;int ncom_radar0=0;
	...
	//Indicator array pointing to the module-variable which are to 
	//be written to 'combus' 'packets'
	int *radar_com_ind=nullptr; int radar_com_count=0;
	...
	//declaring Table pointer as temporary storage of a single table
	Table *table=nullptr;
	//declaring Datadeck 'rockettable' that stores target_rocket trajectory data
	Datadeck rocket_traj;
	//declaring Datadeck 'missiletable' that stores missile trajectory data
	Datadeck missile_traj;
