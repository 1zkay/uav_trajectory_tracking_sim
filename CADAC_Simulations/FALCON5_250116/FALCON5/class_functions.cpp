...
Plane::Plane(Module *module_list,int num_modules)
{
	...
	//zeroeing module-variable array
	for(int i=0;i<NPLANE;i++)plane[i].init("empty",0," "," "," "," ");
	//calling initializer modules to build 'flat3' and 'plane' arrays
	...
	//sizing module-variable array 'com_plane'
	sizing_arrays();

	//allocating dynamic memory to the module-variable arrays
	plane5=new Variable[nplane5];		
	if(!plane5){cerr<<"*** Error: plane5[] allocation failed *** \n";system("pause");exit(1);}

	scrn_plane5=new Variable[nscrn_plane];
	if(!scrn_plane5){cerr<<"*** Error: scrn_plane5[] allocation failed *** \n";system("pause");exit(1);}

	plot_plane5=new Variable[nplot_plane];
	if(!plot_plane5){cerr<<"*** Error: plot_plane5[] allocation failed *** \n";system("pause");exit(1);}
	...
	//building 'plane5' array (compacting and merging 'flat3' and 'plane' arrays)
	vehicle_array();

	//building 'scrn_plane5' array from 'plane5' array
	scrn_array();

	//building 'plot_plane5' array from 'plane' array
	plot_array();
