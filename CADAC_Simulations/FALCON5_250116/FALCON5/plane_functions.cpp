...
///////////////////////////////////////////////////////////////////////////////
//Determining dimensions of arrays: 'plane5', 'scrn_plane5', 'plot_plane5'
//and 'com_plane'
// 
//Out to Plane:: nplane5, nscrn_plane, nplot_plane, ncom_plane,
//	flat3_scrn_count, plane_scrn_count, flat3_plot_count, plane_plot_count 		 ,
//
...
void Plane::sizing_arrays()
{
	...
	//initialize 'Flat3' and 'Plane' member variables
	flat3_scrn_count=0;
	plane_scrn_count=0;
	flat3_plot_count=0;
	plane_plot_count=0;
	...
	nplane5=flat3_full_count+plane_full_count;
	nscrn_plane=flat3_scrn_count+plane_scrn_count;
	nplot_plane=flat3_plot_count+plane_plot_count;
}
///////////////////////////////////////////////////////////////////////////////
//Building 'plane5' module-array by eliminating empty slots in 'flat3' and 'plane'
...
void Plane::vehicle_array()
{
	...
	//load nonempty slots from flat3 array into plane5 array
	int k=0;
	for(i=0;i<NFLAT3;i++)
	{
	...
	//load nonempty slots from plane array into plane5 array	
	int m=0;
	for(i=0;i<NPLANE;i++)
	{
...
///////////////////////////////////////////////////////////////////////////////
//Building 'scrn_plane5' module-array from 'plane' array by keying on the word 'scrn'
//
//Output: Plane::scrn_plane5[] 
//
...
void Plane::scrn_array()
{
	...
	for(int i=0;i<nplane5;i++)
	{
		buff=plane5[i].get_out();
		if(strstr(buff,key))
		{
			scrn_plane5[k]=plane5[i];
			k++;
		}
	}
}
///////////////////////////////////////////////////////////////////////////////
//Building 'plot_plane5' module-array from 'plane' array by keying on the word 'plot'
//
//Output: Plane::plot_plane5[] 
//
//001214 Created by Peter H Zipfel
//011129 Adapted to AIM5 simulation, PZi
//231113 Modified FALCON5, PZi
...
void Plane::scrn_banner()
{
	...
	for(int i=0;i<nscrn_plane;i++)
	{
		cout.setf(ios::left);

		buff1=scrn_plane5[i].get_name();
...
//Assigning initial values to module-variables of 'flat3' and 'plane' arrays
//Reading aero and propulsion decks
//
//The first plane object 'input.asc' reads until the first 'END' after
//'FALCON5'. The second plane object reads untile the second 'END', etc until
//the data for all plane objects are read
//
//Output:	plane_name ('Plane' data member)
//			flat3[] data values (Flat3 data member)
//			plane[] data values ('Plane' data member)
//			aero-tables ('Plane' data members)
//			prop-tables ('Plane' data members)
...
void Plane::vehicle_data(fstream &input)
{
	...
	char *buff=nullptr;			//name of module-variable
	...
	input.getline(plane_name,CHARL,'\n');
...
///////////////////////////////////////////////////////////////////////////////
//Building index array of those 'flat3[]' and plane[] variables 
//that are output to screen  
//
//Output: Plane::flat3_scrn_ind[], plane_crn_ind[] 
//
...
void Plane::plot_banner(ofstream &fplot,char *title)
{
	...
	fplot<<"1"<<title<<" '"<<plane_name<<" ' "<< __DATE__ <<" "<< __TIME__ <<"\n";
	
	//determining the number vector variables
	for(i=0;i<nplot_plane;i++)
	{
		buff1=plot_plane5[i].get_name();
		if(isupper(buff1[0])) m++;
	}
	//increase number of variables by vector components
	int nvariables=nplot_plane+2*m;
...
///////////////////////////////////////////////////////////////////////////////
//Building index array of those 'flat3[]' and plane[] variables that are  
//output to 'ploti.asc'
//
//Output: Plane::flat3_plot_ind[], plane_plot_ind[] 
//
//001213 Created by Peter H Zipfel
//011129 Adapted to AIM5 simulation, PZi
//231113 Modified FALCON5, PZi
...
//011026 Making event_epoch a 'Plane' member for 'stat.asc' file usage, PZi
//011129 Adapted to AIM5 simulation, PZi
//231113 Modified FALCON5, PZi
///////////////////////////////////////////////////////////////////////////////
void Plane::event(char *options)
{
	Variable *watch_variable_ptr=nullptr;
	...
	if(event_epoch)
	{
		...
		{
			cout<<" *** Event #"<<nevent+1<<'\t'<<plane_name<<'\t'<<"time = "<<time
...
//Listing 'plane' and 'flat3' module-variable arrays
//
...
//231113 Modified FALCON5, PZi
///////////////////////////////////////////////////////////////////////////////
void Plane::document(ostream &fdoc,const char *title,Document *doc_plane)
{
	fdoc<<"*********************************************************************************************************************\n";
	fdoc<<"********************************************** FALCON5 **************************************************************\n";
	fdoc<<"*********************************************************************************************************************\n";
...
//	fdoc<<"01234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456\n";
	...
	//building doc_plane[] for documenting 'input.asc' and eliminating 'empty' slots
	int counter=0;
	for(int i=0;i<NPLANE;i++){
		if(strcmp(plane[i].get_name(),"empty")){
			doc_plane[counter].put_doc_offset(counter);
			doc_plane[counter].put_name(plane[i].get_name());
			doc_plane[counter].put_type(plane[i].get_type());
			doc_plane[counter].put_def(plane[i].get_def());
			doc_plane[counter].put_mod(plane[i].get_mod());
			counter++;
		}
	}
	for(int i=0;i<NFLAT3;i++){
		if(strcmp(flat3[i].get_name(),"empty")){
			doc_plane[counter].put_doc_offset(counter);
			doc_plane[counter].put_name(flat3[i].get_name());
			doc_plane[counter].put_type(flat3[i].get_type());
			doc_plane[counter].put_def(flat3[i].get_def());
			doc_plane[counter].put_mod(flat3[i].get_mod());
			counter++;
		}
	}
}
