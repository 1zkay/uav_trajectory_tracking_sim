...
//					&num_hyper, number of hyper vehicles					
...
void number_objects(fstream &input,int &num_vehicles,int &num_hyper,int &num_target
					,int &num_satellite)
{
	char read[CHARN]={};
	char line_clear[CHARL]={};
	...
	//searching for # of hyper,target and satellite objects
	...
	do{
		input>>read;
		input.getline(line_clear,CHARL,'\n');
		if (!strcmp(read,"HYPER5")) num_hyper++;
		if (!strcmp(read,"TARGET3")) num_target++;
		if (!strcmp(read,"SATELLITE3")) num_satellite++;

	}while((num_hyper+num_target+num_satellite)<num_vehicles);
					...
					,int num_satellite)				   
{
	...
	if(!strcmp(temp,"HYPER5"))
	{
		//the pointer 'obj' is allocated the type 'Hyper' 
		try {obj=new Hyper(module_list,num_modules,num_target,num_satellite);}		
		catch(bad_alloc xa){cout<< "*** Error:'obj' allocation of 'Hyper' failed ***\n";exit(1);}
		obj->set_name("HYPER5");
	}
...
///////////////////////////////////////////////////////////////////////////////
void merge_plot_files(string *plot_file_list,int num_hyper,const char *title)
{	
	char line_clear[CHARL]={};
	char line[CHARL]={};
	char buff[CHARN]={};
	...
	try {file_istream_list=new ifstream[num_hyper];}		
...
//to be appended, limiting hyper, target and satellite vehicles to 14 each (e.g.: SBEG1_t1)
...
void traj_banner(ofstream &ftraj,Packet *combus,const char *title,int num_vehicles,
				 int nhyper,int ntarget,int nsatellite)
{
	...
	int ncom_hyper5(0); //number of module variables in 'com_hyper5'
	...
	Variable *data_hyper=NULL; //Variable array stored in 'Packet data' of type Hyper
	...
	//number of data elements in 'com_hyper', 'com_target', and 'com_satellite'
	ncom_hyper5=combus[i_hyper].get_ndata();
	...
	//determining the total number of vector variables in com_hyper5 com_target and com_satellite3
	for(j=0;j<ncom_hyper5;j++)
	{
	...
	int nvariables=ncom_hyper5*nhyper+ncom_target3*ntarget+ncom_satellite3*nsatellite
		+2*(p*nhyper+q*ntarget+r*nsatellite)-(nhyper+ntarget+nsatellite-1);
	...
	for(i=0;i<num_vehicles;i++)
	{
		//determining whether 'Hyper', 'Target', or 'Satellite' object
		id=combus[i].get_id();
		hyper_object=(int)id.find("h");
		target_object=(int)id.find("t");
		satellite_object=(int)id.find("s");
		if(!hyper_object)
		//'Hyper' object
		{
			for(int k=0;k<ncom_hyper5;k++)
			{
...

{
	...
	//determining the number of hyper, target and satellite packets
	for(i=0;i<num_vehicles;i++)
	{
		id=combus[i].get_id();
		loc=(int)id.find("h");
...
//060524 Including satellites, PZi
///////////////////////////////////////////////////////////////////////////////
void document_input(Document *doc_hyper5,Document *doc_target3,Document *doc_satellite3)
{
	...
	input.setf(ios::left);
	do{
		fcopy>>buffo;;//buffo gets HYPER5, ENDTIME STOP and //comments

		//finding HYPER5 object
		if(!strcmp(buffo,"HYPER5")){
			input<<'\t'<<buffo;		//writes HYPER5 line
			fcopy.getline(line_clear,CHARL,'\n');
			input<<line_clear<<'\n';

			//inside HYPER5 loop until 'END' is reached
			do{
				fcopy>>buffn;
				int dum=1;
				...
				//inserting 'END' with only one tab
					...
					def_found=false; 
					for(int k=0;k<(NROUND3+NHYPER);k++){
						if(!strcmp(doc_hyper5[k].get_name(),"end_array"))break;
						if(!strcmp(doc_hyper5[k].get_name(),buffn)){
							fcopy.getline(line_clear,CHARL,'\n'); 							
							input<<"    //";
							if(!strcmp(doc_hyper5[k].get_type(),"int"))input<<"'int' ";
							input<<doc_hyper5[k].get_def();
							input<<"  module ";
							input<<doc_hyper5[k].get_mod();
							input<<'\n';
							*buff_stoch=NULL;
							def_found=true;
						}
		...
		}//end of HYPER5 has been reached
...
//////////////////////////////////////////////////////////////////////////////
void parse_plot_traj_csv(string *files, int num_hyper, bool merge, string type)
{
