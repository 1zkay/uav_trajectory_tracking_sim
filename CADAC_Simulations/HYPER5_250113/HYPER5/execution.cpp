...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_hyper,int num_target,int num_satellite,ofstream &ftraj,const char *title,bool traj_merge);
...
void parse_plot_traj_csv(string *plot_files, int num_hyper, bool merge, string type);
...
int main(void) 
{
	...
	Document *doc_hyper5=NULL;  //array for documenting HYPER5 module-variables of 'input.asc'
	...
	bool document_hyper5=false; //true if array doc_hyper5 was created
	...
	number_objects(input,num_vehicles,num_hyper,num_target,num_satellite);
	...
	for(int i=0;i<num_vehicles;i++)
	{
		...
		if(!strcmp(vehicle_name,"HYPER5")&&one_screen_banner)
		{
		...
		//writing initial block of data to screen after module initialization
		if(strstr(options,"y_scrn"))
		{
		...
		//currently only the HYPER5 plot files contain any data
		if(strstr(options,"y_plot"))
		{
			if(!strcmp(vehicle_name,"HYPER5"))
			{
				char index[CHARN]={}; //
		...
		//composing documentation of 'round3','hyper','satellite' and 'target' module-variables
		//but do it only once for HYPER5, TARGET3 and SATELLITE3 objects
		if(strstr(options,"y_doc"))
		{
			if(!strcmp(vehicle_name,"HYPER5")&&document_hyper)
			{
				document_hyper=false;
				int size=NROUND3+NHYPER;
				try{doc_hyper5=new Document[size];}					
				catch(bad_alloc xa){cout<< "*** Allocation failure of 'doc_hyper5' ***\n";return 1;}								
				vehicle_list[i]->document(fdoc,title,doc_hyper5);
				document_hyper5=true;
			}
		...
		combus[i]=vehicle_list[i]->loading_packet_init(num_hyper,num_target,num_satellite);
	...
	if(strstr(options,"y_traj")&&one_traj_banner)
	{
		...
		traj_banner(ftraj,combus,title,num_vehicles,num_hyper,num_target,num_satellite);
		...
		if(strstr(options,"y_doc")){
			...
			document_input(doc_hyper5,doc_target3,doc_satellite3);
			if(document_hyper5)delete [] doc_hyper5;
	...
	execute(vehicle_list,module_list,sim_time,
			 ...
			 plot_ostream_list,combus,status,num_hyper,num_target,num_satellite,ftraj,title,
			 traj_merge);
	...
	if(strstr(options,"y_csv"))
	{
		if(strstr(options,"y_plot"))
		{
			...
			{
				parse_plot_traj_csv(plot_file_list,num_hyper,1,"plot");
			}
			else{
				parse_plot_traj_csv(plot_file_list,num_hyper,0,"plot");
			}
		}
	...
	while (sim_time<=(end_time+int_step))
	{
		...
		for (int i=0;i<num_vehicles;i++)
		{
			...
			if(health==1)
			{
				...
				combus[i]=vehicle_list[i]->loading_packet(num_hyper,num_target,num_satellite);
