...
PX4 SITL + Gazebo Harmonic + ROS 2 Jazzy 的本地 NED 参数化轨迹跟踪仿真工作区。当前视觉链路采用 **YOLO + BoT-SORT 目标跟踪**，主机 `x500_0` 使用官方 **x500_mono_cam 前向固定单目相机**。视觉链路通过相机内参、固定安装姿态和 PX4 机体姿态计算 LOS，沿 LOS 起步，速度方向与相邻历史样本有效后运行 PNG 制导，向 PX4 发送 NED 速度与 LOS 偏航角，由 PX4 速度、姿态和角速度控制环计算推力。实现边界与验证见 [视觉制导说明](docs/visual_guidance.md)。

当前 `main` 为固定相机版本；原云台版本已上传为 Git 标签 `archive/gimbal-20260907`，旧云台文件和兼容入口已从当前工作树移除。迁移依据、接口与验证见 [固定相机迁移说明](docs/fixed_camera_migration.md)。
...
- ROS 2: Jazzy
- Gazebo: Harmonic
- DDS Agent: **Micro XRCE-DDS Agent v2.4.3**，`/usr/local/bin/MicroXRCEAgent udp4 -p 8888`

当前 PX4 客户端为 2.x，ROS 2 Jazzy 使用 Fast DDS 2.14；Agent 必须使用兼容的 2.x 版本，
不要安装 Agent 3.x 后直接用于本环境。版本对应关系见 [PX4 官方说明](https://docs.px4.io/main/en/middleware/uxrce_dds)。

仿真时间遵循 [PX4 官方统一 Gazebo 时钟配置](https://docs.px4.io/main/en/ros2/user_guide#time-synchronization)：
两个 PX4 启动脚本设置 `PX4_PARAM_UXRCE_DDS_SYNCT=0`；两个 ROS launch 设置 `use_sim_time=true`。
PX4 状态、图像、真值和控制命令都使用仿真时间，不再在应用层转换 DDS 偏移。
更新后必须重启双机 PX4 和 ROS 链路，不能与旧时间配置混用。
`start_simulation_clock.sh` 独立启动唯一的 `/clock` 桥接，与 Gazebo 一起保持运行；
主机和目标机业务 launch 只消费时钟，重启视觉链路不会中断目标机时钟。
...
先启动 QGroundControl 并保持运行，等待它连接两个 SITL 实例。清理 PX4 编译目录后，
默认参数下缺少地面站心跳会使解锁被拒绝（`No connection to the ground control station`）：

```bash
/home/zk/QGroundControl-x86_64.AppImage
```
...
终端 3，启动公共仿真时钟，整个仿真只启动一次，直到仿真结束再关闭：

```bash
cd /home/zk/uav_trajectory_tracking_sim
./scripts/start_simulation_clock.sh
```

如果设置了 `PX4_GZ_WORLD`，此终端与 PX4/Gazebo 终端必须使用相同值。
单独运行目标轨迹时也需要此服务，无需启动主机视觉链路。

如果只做主机轨迹跟踪，终端 4 启动主机轨迹跟踪、相机桥接、YOLO + BoT-SORT 跟踪、日志记录和 RViz 可视化发布节点：
...
终端 5，启动目标轨迹节点，使用终端 3 提供的公共时钟开始控制：
...
终端 6，启动主机视觉拦截链路。该入口会启动相机桥接、YOLO + BoT-SORT、固定相机目标观测、主机 Gazebo truth 日志桥接和 `visual_pursuit_interceptor`：
...
终端 7 可选，用于查看轨迹、目标点、规划路径和实际飞行尾迹：
...
终端 8 可选，用于通过 Foxglove 查看 ROS 2 话题、曲线、图像和运动状态：
...
相机链路由共享的 `camera.launch.py` 启动官方桥接和静态 TF 发布器，直接输出
`/x500_0/camera/image_raw` 与 `/x500_0/camera/camera_info`，使用光学 frame 并保留采集时间。
图像桥接固定为 RELIABLE、深度 1；旧 `CAMERA_IMAGE_BRIDGE_QOS` / `camera_image_bridge_qos` 选项已移除。
...
`src/uav_trajectory_tracking/config/target_trajectory_hold.yaml`。两者坐标系都是 PX4 本地 NED：

- `x`: 北/前
- `y`: 东/右
- `z`: 下，起飞点上方 5 m 写作 `-5.0`

只改主机默认轨迹时修改 `trajectory_hold.yaml` 或用 `TRAJECTORY_FILE=...` 覆盖；只改目标机轨迹时修改 `target_trajectory_hold.yaml` 或用 `TRAJECTORY_FILE=...` 覆盖。
...
`target_trajectory_hold.yaml`，用于定点目标调试。若需要持续横向运动目标，可把
`TRAJECTORY_FILE` 指向 `src/uav_trajectory_tracking/config/target_trajectory_linear.yaml`。
...
受风模型和真值 odometry，不修改 PX4 原始 `x500_base` / `x500_mono_cam`。固定相机在项目包装模型中覆盖图像尺寸为 1920×1080，其余沿用官方传感器设置。
...
位姿为 `(0.12, 0.03, 0.002, 0, 0, 0)`（相对 `base_link`，FLU），图像为 1920×1080、30 Hz。
尺寸由 `px4_overlays/models/x500_mono_cam_trajectory_wind/model.sdf` 的 include 参数覆盖，
CameraInfo 内参由 Gazebo 按新尺寸生成；水平视场角保持 1.74 rad，16:9 图像的垂直视场比原 4:3 更窄。
...
视觉入口增加 `fixed_camera_target_tracker`，从实际 `CameraInfo.K` 计算目标单位视线。
`/x500_0/fixed_camera_target_tracker/bearing` 为光学系单位向量（x 右、y 下、z 前），时间戳保留 Gazebo 图像采集时间。
`tracking_active` 表示新鲜有效观测；`lock_active` 表示同一目标已连续观测达到确认时长，
不要求目标先居中。未收到内参、观测过期或 frame 不匹配时不输出可用观测。

控制器只使用固定相机观测和 PX4 机体姿态构造视线方向。
到达初始悬停点后，无目标时保持 XY，结合机体偏航与上下高度扫描搜索；
高度以搜索开始时的实际 Z 为中心，当前配置幅度 4 m、周期 12 s，Z 设定值限制在本地 NED 的 −8～−2 m。
`search_vertical_amplitude_m: 0.0` 可关闭升降。起飞、前往初始悬停点和 COAST 阶段不升降搜索。
收到新鲜观测后停止升降，等待锁定时保持当前高度，并通过 `yaw_mode: face_los` 转向目标；
固定相机的画面会随无人机俯仰和横滚变化，不具备云台的独立稳像能力。

配置文件：

- `config/yolo_tracking.yaml`：检测、跟踪和推理参数。
- `config/fixed_camera_tracking.yaml`：单目标类别、置信度、超时及确认时长（忽略 track ID 变化）。
- `config/visual_interception.yaml`：固定外参、图像观测、偏航/高度搜索及速度方向制导、速度参考参数。

以上路径相对 `src/uav_trajectory_tracking/`。可使用 `FIXED_CAMERA_CONFIG_FILE` 覆盖观测参数文件。
普通轨迹入口提供相机桥接、YOLO 跟踪和图像标注；视觉拦截使用独立的 `start_visual_interception.sh` 入口。
...
当前默认 `enabled: false`，启动时不加载风场及 `WindEffects` 插件，恒定风和随机扰动均关闭。
重新启用时，配置中的 `[1.0, 0.0, 0.0]` 表示 1 m/s 向东基础风。风场使用 Gazebo 标准的
`<wind>` + `WindEffects` 结构，`wind_effects` 下的字段和 Gazebo SDF 标签保持一致；
其中 `time_for_rise` / `period` 单位是秒，方向扰动 `amplitude` 单位是弧度。
启动前改 YAML 会自动渲染到 Gazebo world；修改开关后需重新启动 Gazebo 才生效。
已启用风场的仿真运行中，也可以临时改基础风速：
...
- `gust_east_3ms.yaml`: 3 m/s 向东基础风，带轻微阵风。
...
视觉入口还在同一运行目录写入 `visual_control.csv`，记录控制周期的仿真时间、模式、LOS、实际/期望速度方向及速度设定值；普通轨迹入口不生成该文件。
...
定量误差分析使用新增的 `state_comparison.csv`：按 DDS 同步还原的仿真采样时间配对，
先修正真值机体参考点，再按各机 PX4 全局参考对齐本地原点。原始 CSV 的接收时间仅供追溯，
不能当作精确采样时间对齐依据。在线 PX4/truth/error 三个话题共用配对采样时间，
无可信同步、原点或有效样本时不发布对应误差；状态见 `/x500_0/state_compare/status`。
配置、坐标契约及目标机用法见 [Logger 对齐说明](docs/logger_alignment.md)。
...
- `/fmu/out/vehicle_odometry`: 主机 PX4 里程计估计，包含位置、姿态、速度、body FRD 角速度和方差。视觉制导与曝光姿态配对统一使用此话题；新样本更新制导，发布周期保持指令；追踪时沿 LOS 起步，速度方向与相邻历史样本有效后启用 PNG，详见 [视觉制导说明](docs/visual_guidance.md)。
...
- `/x500_0/state_compare/truth_position_ned`: 主机 Gazebo truth 经机体参考点和 PX4 原点对齐后的 NED 位置。
...
- `/x500_0/state_compare/truth_acceleration_ned`: 主机 Gazebo truth 修正参考点后的 NED 速度差分得到的加速度，单位为 `m/s^2`。
...
- `/fmu/in/trajectory_setpoint`: 普通轨迹控制，以及视觉控制的起飞、搜索、PURSUIT 速度追踪和 COAST 减速输入。
...
- `/x500_0/fixed_camera_target_tracker/bearing`：光学系单位视线，保留图像采集时间戳。
- `/x500_0/fixed_camera_target_tracker/tracking_active`：有新鲜有效目标观测。
- `/x500_0/fixed_camera_target_tracker/lock_active`：同一目标连续确认完成。
- `/x500_0/visual_pursuit_interceptor/diagnostics`: 视觉拦截诊断，包含 `state`、`pursuing`、`velocity_control_active`、`bearing_fresh`、`los_filter_*`、`closing_speed_mps`、`los_ned_*`、实际/期望速度方向、速度设定值及状态新鲜度。
...
确认 CameraInfo 为 1920×1080、fx/fy 有效，观测与内参 frame 一致，
`vehicle_state_fresh=true`、`bearing_fresh=true`。检测需连续满足
`fixed_camera_tracking.yaml` 的 `lock_confirm_s`，且采集到控制的延迟小于共享 `observation_timeout_s`（仅在 `fixed_camera_tracking.yaml` 定义）与 `los_filter_max_prediction_s` 中的较小值。
短暂掉锁保留原 `coast_on_lock_loss` 衰减行为，超时后保持 XY 并进行偏航/高度搜索。
